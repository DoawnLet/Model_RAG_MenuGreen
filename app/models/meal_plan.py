...
class NutritionTargets(BaseModel):
    ...
    def get_meal_targets(
        self, meal_type: Literal["breakfast", "lunch", "dinner", "snack"]
    ) -> dict:
        """
...
class RecipeIngredient(BaseModel):
    ...
    calories: Optional[float] = Field(
        default=None, description="Calories của nguyên liệu này"
    )
    protein: Optional[float] = Field(
        default=None, description="Protein (g) của nguyên liệu này"
    )
...
class AdaptedRecipe(BaseModel):
    ...
    ghi_chu: Optional[str] = Field(
        default=None, description="Ghi chú thêm (tips, substitutions)"
    )
...
class WeeklyMealPlanAllocation(BaseModel):
    ...
    allocations: list[DailyAllocation] = Field(
        description="Danh sách phân bổ cho 7 ngày", min_length=7, max_length=7
    )
...
class ShoppingListItem(BaseModel):
    ...
    ghi_chu: Optional[str] = Field(
        default=None, description="Ghi chú (ví dụ: optional, có thể thay thế)"
    )
...
class MealPlanOutput(BaseModel):
    ...
    thong_tin_nguoi_dung: UserInfo = Field(
        description="Thông tin và mục tiêu người dùng"
    )
    thuc_don_7_ngay: list[DailyMealPlan] = Field(description="Thực đơn 7 ngày (28 bữa)")
    danh_sach_mua: list[ShoppingListItem] = Field(
        description="Danh sách nguyên liệu cần mua"
    )
