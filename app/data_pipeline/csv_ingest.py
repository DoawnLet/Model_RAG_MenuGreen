...
async def ingest_csv(csv_path: str, limit: int = 100):
    ...
    for _, row in df.iterrows():
        try:
            ...
            ingredients = parse_list_string(
                str(row.get(COL_MAPPING["ingredients"], "[]"))
            )
            instructions_list = parse_list_string(
                str(row.get(COL_MAPPING["steps"], "[]"))
            )
            ...
            vector_text = (
                f"Recipe: {name}. Description: {desc}. Keywords: {', '.join(tags)}"
            )
